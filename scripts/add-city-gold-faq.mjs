// One-off: add a customer-intent FAQ block + FAQPage schema to the
// History of City Gold article pages (EN+BN). Questions are what buyers
// actually ask when shopping city gold ornaments. Answers use only verified
// metallurgy facts or eMarket247's published practice — no invented
// sourcing, warranty, or delivery claims.
...
const ld = (lang) => ({
  ...
  "@type": "FAQPage",
  "@id": `https://emarket247.shop/${lang}/history-of-city-gold/#faq`,
  inLanguage: lang,
...
for (const lang of ["en", "bn"]) {
  const f = `public_html/${lang}/history-of-city-gold/index.html`;
  ...
  const block = `<section class="faq-section wrap"><p class="eyebrow">${eyebrow[lang]}</p><h2>${heading[lang]}</h2>${items}<p class="faq-more">${lang === "en" ? "Have a question about a specific design? Ask us on WhatsApp in Bangla or English — we reply with availability and current details before you decide." : "নির্দিষ্ট কোনো ডিজাইন নিয়ে প্রশ্ন আছে? হোয়াটসঅ্যাপে বাংলা বা ইংরেজিতে জিজ্ঞেস করুন — সিদ্ধান্তের আগেই প্রাপ্যতা ও বর্তমান বিবরণ জানিয়ে দেব।"}</p></section>`;
  // Insert before the closing care-note block's parent prose end: after </section> of .prose
  ...
  // Merge FAQPage into the article's @graph
