#!/usr/bin/env node
/**
 * Update PDP generator renderHeader to match the new 3-row global header.
 * Replaces lines 64-86 with the new ICON constants + 3-row renderHeader template.
 */
...
const TARGET = path.join(ROOT, 'scripts', 'generate-product-detail-pages.mjs');

const src = readFileSync(TARGET, 'utf8');

// Find the line where renderHeader function starts (line 64)
const lines = src.split('\n');
const startIdx = lines.findIndex(l => l.trim().startsWith('function renderHeader(lang)'));
if (startIdx === -1) {
  console.error('❌ Could not find renderHeader function');
  process.exit(1);
}

// Find the closing brace of renderHeader (line 86 ends with `}`)
let endIdx = startIdx;
let braceDepth = 0;
let foundOpen = false;
...
console.log(`Found renderHeader at lines ${startIdx + 1}-${endIdx + 1}`);

const newRenderHeader = `// SVG Icons (stroke-based, matching the cart icon style)
const ICON = {
...
console.log(`✅ Updated ${TARGET} with 3-row header (ICON constants + renderHeader)`);
