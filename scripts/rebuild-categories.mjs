/**
 * RETIRED — category-index writer, superseded by the corrected taxonomy.
 *
 * This was a one-off that rebuilt the EN and BN categories index pages: a slim
 * hero, a nine-tile image grid reusing the homepage's category-tile markup, a
 * merged "how to choose" block, and a bottom CTA.
 *
 * It is retired because it replaces everything from <main> to </main> with its
 * own template, and that template predates scripts/add-pre-footer-cta.mjs.
 * Rerunning it would therefore delete the WhatsApp CTA section from both
 * category-index pages. Its tiles also still name a product of the wrong
 * category for Earrings, Pendants and Rings.
 *
 * Category identity lives in public_html/assets/data/catalog.taxonomy.json and
 * is enforced by scripts/validate-public-html.mjs. This script intentionally
 * writes nothing.
 */
import { readFileSync, writeFileSync } from "node:fs";

console.error(
  "Category-index generation is disabled: this writer replaces the whole <main> and would delete the WhatsApp CTA section."
);
console.error(
  "Use catalog.taxonomy.json as the canonical mapping and run npm test before publishing catalogue changes."
);
// A hard stop, not process.exitCode: the template below is kept as the record,
// so the exit must happen before the write loop further down can be reached.
process.exit(1);

// ── Historical record, not executed ──────────────────────────────────────────
// The template below is retained so the file still documents the markup the two
// live pages carry. The three category-tile photos that contradicted the
// corrected taxonomy have been brought in line with the live pages, and nothing
// after the exit above can run.
...
<section class="category-section wrap" aria-label="All jewellery categories">
  <div class="category-grid category-product-grid">
    ...
    <a href="/en/categories/pendants/" class="category-tile has-media"><figure><img src="/assets/images/emarket247-gold-winged-flower-pendant.webp" alt="eMarket247 pendants category — gold-tone winged flower pendant" width="1600" height="1600" loading="lazy" decoding="async"></figure><div class="category-tile-body"><div class="category-tile-header"><h3>Pendants</h3><small>লকেট <b>↗</b></small></div><p class="category-desc">Small details that make a personal statement.</p></div></a>
    <a href="/en/categories/rings/" class="category-tile has-media"><figure><img src="/assets/images/products/emarket247-gold-interwoven-pattern-ring.webp" alt="eMarket247 rings category — gold-tone interwoven ring" width="1600" height="1600" loading="lazy" decoding="async"></figure><div class="category-tile-body"><div class="category-tile-header"><h3>Rings</h3><small>আংটি <b>↗</b></small></div><p class="category-desc">Versatile rings for everyday styling, gifting and occasions.</p></div></a>
