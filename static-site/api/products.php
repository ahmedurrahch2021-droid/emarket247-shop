...
if ($method === 'POST') {
    ...
    if (empty($titleEn)) {
        sendJsonResponse(['status' => 'error', 'message' => 'Product title in English is required.'], 400);
    }
...
if ($method === 'PUT') {
    ...
    if ($id <= 0) {
        sendJsonResponse(['status' => 'error', 'message' => 'Valid product ID is required.'], 400);
    }
    ...
    if (empty($fields)) {
        sendJsonResponse(['status' => 'error', 'message' => 'No fields provided to update.'], 400);
    }
    ...
    sendJsonResponse(['status' => 'success', 'message' => 'Product updated successfully.']);
}
...
if ($method === 'DELETE') {
    ...
    sendJsonResponse(['status' => 'success', 'message' => "Product #{$id} has been deleted."]);
}
